 ...
 * |     8    32    64   256|
 * |     2     4    32   128|
 * |     4     2     8    16| ->0x4312 2^4=16 2^3=8 2^1=2 2^2=4
 * +------------------------+
 *
 */
class board {
...
private:
	...
	int opcode; //a
	int score; //r
	float esti; //value to estimate the goodness of the board(the summation of the weight sum of all tuple and board)
};
...
public:
	...
	float estimate(const board& b) const {
		...
		// for(int j=0; j<num; ++j){
		// 	float exp_2=0, exp_4=0;
		// 	board b_p2 = b;
		// 	b_p2.set(space[j], 1);
		// 	for (feature* feat : feats) {
		// 		exp_2 += feat->estimate(b_p2);
		// 	}
		// 	board b_p4 = b;
		// 	b_p4.set(space[j], 2);
		// 	for (feature* feat : feats) {
		// 		exp_4 += feat->estimate(b_p4);
		// 	}
		// 	value += 1.0/num * 0.9 * exp_2 + 1.0/num * 0.1 * exp_4;
		// }
		// std::cout<<"estimate value"<<value<<std::endl;
	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				// TODO
				// record all empty tile after actions
				board b_p2 = move->after_state();
				board b_p4 = move->after_state();
				int space[16]={0}, num=0;
				for(int i=0; i<16; ++i){
					if(b_p2.at(i)==0)
						space[num++]=i;
				}
				//calculate the expectation of all possible conditions
				float exp=0;
				for(int j=0; j<num; ++j){
					// board b_p4 = move->after_state();
					b_p2.set(space[j], 1);
					exp += 1.0/num * 0.9 * estimate(b_p2);
					b_p4.set(space[j], 2);
					exp += 1.0/num * 0.1 * estimate(b_p4);
				}
				exp = 0.25*exp + move->reward();
				//set the state's value as the reward + the expectation of the state after random popup
	...
	void update_episode(std::vector<state>& path, float alpha = 0.1) const {
		// TODO
		//perform TD state update
		
		float exact = 0;
		for(path.pop_back(); path.size(); path.pop_back()){
			state& move = path.back();
			float error = exact + move.reward() - move.value();
	...
	void make_statistic(size_t n, const board& b, int score, int unit = 1000) {
		...
		if (n % unit == 0) { // show the training process
			...
			std::ofstream file;
			file.open("state_v2_score_log.txt", std::ios_base::app);
			file<<mean<<"\t";
			file<<max<<"\n";
			file.close();
...
int main(int argc, const char* argv[]) {
	...
	// initialize the features 4*6(4 feature each with 6 tiles)
