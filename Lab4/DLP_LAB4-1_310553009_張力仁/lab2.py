...
class EEGNet(nn.Module):
    def __init__(self, activation=None, dropout=0.25):
        ...
        self.depthwiseConv = nn.Sequential(
            nn.Conv2d(
                8, 16, kernel_size=(2,1),
                stride=(1,1), groups=8, bias=False
            ),
            nn.BatchNorm2d(16),
            activation(),
...
def runModels(
    ...
    **{key+"_test" : [] for key in models}
    ...
    optimizers = {
        key: optimizer(value.parameters(), lr=learning_rate, weight_decay=0.0009) 
    ...
    for epoch in range(epoch_size):
        ...
        if epoch % 30 == 0:
            tmp = AccuracyResult()
            tmp.add("EEGNet", Accs, show=False)
            tmp.show()
...
if __name__ == "__main__":
    ...
    Accs = runModels(models, epoch_size=420, batch_size=64, learning_rate=1e-3, show=True)
    ...
    # }
    # Accs = runModels(models, epoch_size=300, batch_size=64, learning_rate=1e-3, show=True)
